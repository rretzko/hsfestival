    ...
    <div class="flex row text-white space-x-2">
        ...
        <div id="page content" class="bg-indigo-500 w-full h-full ml-6 py-10 px-4 text-xl">
            <header class="uppercase mb-4">
                Event Ensembles: Venue + Date Assignment
            </header>
                                ...
                                <table class="min-w-full divide-y divide-gray-200 text-black">
                                    ...
                                    @forelse($ensembles AS $ensemble)
                                        <tr class="@if($loop->odd) bg-gray-100 @else bg-white @endif">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                @foreach($ensemble->useroptionsvenues() AS $useroptionvenue)
                                                    @if($useroptionvenue->preference)
                                                        <span class="@if($ensemble->hasAssignment && $ensemble->assignedVenueid == $useroptionvenue->venue->id) font-bold @endif">
                                                            {{ $useroptionvenue->venue->shortname.' ('.$useroptionvenue->venue->startDateMd.')' }}<br />
                                                        </span>
                                                    @endif
                                                @endforeach
                                            </td>
                                            ...
                                            {{-- INPUT FORM --}}
                                            <form method="post" action="{{ route('eventmanagement.scheduling.days.update') }}">
                                                @csrf
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                    <select name="venue_id">
                                                        @foreach($ensemble->useroptionsvenues() AS $useroptionvenue)
                                                            @if($useroptionvenue->preference)
                                                                <option value="{{ $useroptionvenue->venue_id }}"
                                                                    @if($ensemble->assignedVenueid == $useroptionvenue->venue_id) SELECTED @endif
                                                                >
                                                                    {{ $useroptionvenue->venue->shortname.' ('.$useroptionvenue->venue->startDateMd.')' }}<br />
                                                                </option>
                                                            @endif
