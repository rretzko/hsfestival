<div>
    <div class="card-controls sm:flex">
        <div class="w-full sm:w-1/2">
            Per page:
            <select wire:model="perPage" class="form-select w-full sm:w-1/6">
                @foreach($paginationOptions as $value)
                    <option value="{{ $value }}">{{ $value }}</option>
                @endforeach
            </select>

            @can('ensemble_delete')
                <button class="btn btn-rose ml-3 disabled:opacity-50 disabled:cursor-not-allowed" type="button" wire:click="confirm('deleteSelected')" wire:loading.attr="disabled" {{ $this->selectedCount ? '' : 'disabled' }}>
                    {{ __('Delete Selected') }}
                </button>
            @endcan

            @if(file_exists(app_path('Http/Livewire/ExcelExport.php')))
                <livewire:excel-export model="Ensemble" format="csv" />
                <livewire:excel-export model="Ensemble" format="xlsx" />
                <livewire:excel-export model="Ensemble" format="pdf" />
            @endif




        </div>
        <div class="w-full sm:w-1/2 sm:text-right">
            Search:
            <input type="text" wire:model.debounce.300ms="search" class="w-full sm:w-1/3 inline-block" />
        </div>
    </div>
    <div wire:loading.delay>
        Loading...
    </div>

    <div class="overflow-hidden">
        <div class="overflow-x-auto">
            <table class="table table-index w-full">
                <thead>
                    <tr>
                        <th class="w-9">
                        </th>
                        <th class="w-28">
                            {{ trans('cruds.ensemble.fields.id') }}
                            @include('components.table.sort', ['field' => 'id'])
                        </th>
                        <th>
                            {{ trans('cruds.ensemble.fields.user') }}
                            @include('components.table.sort', ['field' => 'user.name'])
                        </th>
                        <th>
                            {{ trans('cruds.ensemble.fields.school') }}
                            @include('components.table.sort', ['field' => 'school.name'])
                        </th>
                        <th>
                            {{ trans('cruds.ensemble.fields.event') }}
                            @include('components.table.sort', ['field' => 'event.name'])
                        </th>
                        <th>
                            {{ trans('cruds.ensemble.fields.name') }}
                            @include('components.table.sort', ['field' => 'name'])
                        </th>
                        <th>
                            {{ trans('cruds.ensemble.fields.conductor') }}
                            @include('components.table.sort', ['field' => 'conductor'])
                        </th>
                        <th>
                            {{ trans('cruds.ensemble.fields.ensembletype') }}
                            @include('components.table.sort', ['field' => 'ensembletype.descr'])
                        </th>
                        <th>
                            {{ trans('cruds.ensemble.fields.auditioned') }}
                            @include('components.table.sort', ['field' => 'auditioned'])
                        </th>
                        <th>
                        </th>
                    </tr>
                </thead>
                <tbody>
                    @forelse($ensembles as $ensemble)
                        <tr>
                            <td>
                                <input type="checkbox" value="{{ $ensemble->id }}" wire:model="selected">
                            </td>
                            <td>
                                {{ $ensemble->id }}
                            </td>
                            <td>
                                @if($ensemble->user)
                                    <span class="badge badge-relationship">{{ $ensemble->user->name ?? '' }}</span>
                                @endif
                            </td>
                            <td>
                                @if($ensemble->school)
                                    <span class="badge badge-relationship">{{ $ensemble->school->name ?? '' }}</span>
                                @endif
                            </td>
                            <td>
                                @if($ensemble->event)
                                    <span class="badge badge-relationship">{{ $ensemble->event->name ?? '' }}</span>
                                @endif
                            </td>
                            <td>
                                {{ $ensemble->name }}
                            </td>
                            <td>
                                {{ $ensemble->conductor }}
                            </td>
                            <td>
                                @if($ensemble->ensembletype)
                                    <span class="badge badge-relationship">{{ $ensemble->ensembletype->descr ?? '' }}</span>
                                @endif
                            </td>
                            <td>
                                <input class="disabled:opacity-50 disabled:cursor-not-allowed" type="checkbox" disabled {{ $ensemble->auditioned ? 'checked' : '' }}>
                            </td>
                            <td>
                                <div class="flex justify-end">
                                    @can('ensemble_show')
                                        <a class="btn btn-sm btn-info mr-2" href="{{ route('admin.ensembles.show', $ensemble) }}">
                                            {{ trans('global.view') }}
                                        </a>
                                    @endcan
                                    @can('ensemble_edit')
                                        <a class="btn btn-sm btn-success mr-2" href="{{ route('admin.ensembles.edit', $ensemble) }}">
                                            {{ trans('global.edit') }}
                                        </a>
                                    @endcan
                                    @can('ensemble_delete')
                                        <button class="btn btn-sm btn-rose mr-2" type="button" wire:click="confirm('delete', {{ $ensemble->id }})" wire:loading.attr="disabled">
                                            {{ trans('global.delete') }}
                                        </button>
                                    @endcan
                                </div>
                            </td>
                        </tr>
                        @empty
                        <tr>
                            <td colspan="10">No entries found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    </div>

    <div class="card-body">
        <div class="pt-3">
            @if($this->selectedCount)
                <p class="text-sm leading-5">
                    <span class="font-medium">
                        {{ $this->selectedCount }}
                    </span>
                    {{ __('Entries selected') }}
                </p>
            @endif
            {{ $ensembles->links() }}
        </div>
    </div>
</div>

@push('scripts')
    <script>
        Livewire.on('confirm', e => {
    if (!confirm("{{ trans('global.areYouSure') }}")) {
        return
    }
@this[e.callback](...e.argv)
})
    </script>
@endpush